...
public class AlunosJpaController implements Serializable {
    ...
    private AlunosJpaController(){
    }
    
    public AlunosJpaController(EntityManagerFactory emf) {
        this.emf = emf;
    }
    private static AlunosJpaController instance;
    ...
    public static AlunosJpaController getInstance(){        
        if(emf == null){
            instance = new AlunosJpaController(Persistence.createEntityManagerFactory("EnsinoMarioPU"));
        }
        return instance;
    }
    ...
    public void create(Alunos alunos) {
        ...
        try {
            ...
            if (idCurso != null) {
                idCurso = em.getReference(idCurso.getClass(), idCurso.getFirst());
    ...
    public void edit(Alunos alunos) throws NonexistentEntityException, Exception {
        ...
        try {
            em = getEntityManager();
            em.getTransaction().begin();
            Alunos persistentAlunos = em.find(Alunos.class, alunos.getFirst());
            ...
            if (idCursoNew != null) {
                idCursoNew = em.getReference(idCursoNew.getClass(), idCursoNew.getFirst());
